...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val suggestions: Observable[Try[List[String]]] = searchTerms.concatRecovered(wikiSuggestResponseStream).timedOut(1)
    ...
    // TO IMPLEMENT
    val selections: Observable[String] = button.clicks.map(_ => suggestionList.listData.head)
    ...
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      x => editorpane.text = x.getOrElse("Error!")
    }
