...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    val list = w.toLowerCase.groupBy(c => c).toList
    val unsortedOccurrences = list map { case (c, n) => (c, n.count(_ == c)) }
    unsortedOccurrences.sorted
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def combinationsInner(c: Char, n: Int): List[Occurrences] = {
      if (n == 0) List(List((c, 0)))
      else List((c, n)) :: combinationsInner(c, n - 1)
    }

    if (occurrences.isEmpty) List(List())
    else {
      for {
        tuples <- combinationsInner(occurrences.head._1, occurrences.head._2)
        tuple <- tuples
        rest <- combinations(occurrences.tail)
      } yield {
        if (tuple._2 > 0) tuple :: rest
        else rest
      }
    }
  }
