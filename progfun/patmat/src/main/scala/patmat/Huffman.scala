...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] =
    chars.distinct.map(c => (c,chars.count(x => x == c)))
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    if (bits.isEmpty) Nil
      ...
      val (char, count) = decodef(tree, bits, 0)
      char :: decode(tree, bits.drop(count))
    }
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    def convertf(tree: CodeTree, table: CodeTable, acc: List[Bit]): CodeTable = tree match {
      case Leaf(c, w) => mergeCodeTables(table, List((c, acc)))
      case Fork(l, r, c, w) => mergeCodeTables(convertf(l, table, acc :+ 0), convertf(r, table, acc :+ 1))
    }

    convertf(tree, Nil, Nil)
  }
